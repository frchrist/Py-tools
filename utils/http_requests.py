...
def post_image(image_path: str, url: str, headers: dict) -> bytes:
    """
    ...
    try:
        # Send the HTTP POST request with the specified headers and image file data
        response = requests.post(url, files={'image_file': open(image_path, 'rb')}, data={'size': 'auto'}, headers=headers)
...
def write_image_to_download(image_buffer, image_path):
	...
	processed_image_path = f"{DOWNLOAD_FOLDER}/{name}-nobg{extention}"
...
def remove_bg(path: str) -> str:
    ...
    try:
        ...
        buffer = post_image(path, REMOVE_URL, REMOVE_HEADERS)
...
if __name__ == "__main__":
	path = "/home/christian/workspace/py-tools/view/assets/face.png"
	buffer = post_image(path, REMOVE_URL, REMOVE_HEADERS)
